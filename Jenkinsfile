    ...
    stage('Build') {
     ...
     stage ('Test') {
		...
		stage ('Retrieve Artifact from Nexus'){
			 steps {
				sh 'mvn org.apache.maven.plugins:maven-dependency-plugin:2.8:get  -DrepoUrl=http://10.0.1.153:8081/nexus/ -Dartifact=org.springframework:gs-spring-boot:0.1.0:jar'
				// script {
